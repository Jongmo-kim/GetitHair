...
ALTER TABLE review
    ADD CONSTRAINT FK_review_shop_no_hairshop_sho FOREIGN KEY (shop_no)
        REFERENCES hairshop (shop_no) ON DELETE CASCADE --같이지우기
/

ALTER TABLE review
    ADD CONSTRAINT FK_review_designer_no_designer FOREIGN KEY (designer_no)
        REFERENCES designer (designer_no) ON DELETE CASCADE --같이지우기
/

ALTER TABLE review
    ADD CONSTRAINT FK_review_customer_no_customer FOREIGN KEY (customer_no)
        REFERENCES customer (customer_no) ON DELETE SET NULL --리뷰는 살리기
/

ALTER TABLE review
    ADD CONSTRAINT FK_review_style_no_style_style FOREIGN KEY (style_no)
        REFERENCES style (style_no) ON  DELETE SET NULL --리뷰 살리기
/
-- 시술 가격 테이블
...
ALTER TABLE style_list
    ADD CONSTRAINT FK_style_list_style_no_style_s FOREIGN KEY (style_no)
        REFERENCES style (style_no) ON DELETE CASCADE
/
-- hairshop Table Create SQL
...
(
    ...
    reserve_title            varchar2(50)     DEFAULT '',
    reserve_status           char(6)          DEFAULT'예약' NOT NULL, 
    reserve_cust_req         VARCHAR2(300)    NULL, 
    reserve_designer_req     VARCHAR2(300)    NULL, 
    reserve_designer_memo    VARCHAR2(300)    NULL, 
    reserve_date             DATE             NOT NULL, 
    reserve_startdate        DATE             NOT NULL,
    reserve_enddate          DATE             NOT NULL, --api특성상 end DATE 추가
...
ALTER TABLE reserve
    ADD CONSTRAINT FK_reserve_customer_no_custome FOREIGN KEY (customer_no)
        REFERENCES customer (customer_no) ON DELETE SET NULL --살리기
/

ALTER TABLE reserve
    ADD CONSTRAINT FK_reserve_designer_no_designe FOREIGN KEY (designer_no)
        REFERENCES designer (designer_no) ON DELETE SET NULL --살리기
/

ALTER TABLE reserve
    ADD CONSTRAINT FK_reserve_shop_no_hairshop_sh FOREIGN KEY (shop_no)
        REFERENCES hairshop (shop_no) on delete set null -- 살리기
/
ALTER TABLE reserve
...
ALTER TABLE designer_list
    ADD CONSTRAINT FK_designer_list_designer_no_d FOREIGN KEY (designer_no)
        REFERENCES designer (designer_no) ON DELETE cascade --헤어샵, 디자이너 탈퇴, 삭제시 디자이너리스트도 같이 삭제
...
ALTER TABLE likes
    ADD CONSTRAINT FK_likes_customer_no_customer_ FOREIGN KEY (customer_no)
        REFERENCES customer (customer_no) ON DELETE SET NULL -- 커스터머 탈퇴시 likes 남음
/
...
CREATE TABLE DESIGNER_PORTFOLIO(
    ...
    DESIGNER_PORTFOLIO_DATE DATE,       -- 글작성날짜
...
BEGIN
    ...
    V_LAST  :=  V_ARR('서울특별시' , '부산시' , '대구시' , '전주시' );
