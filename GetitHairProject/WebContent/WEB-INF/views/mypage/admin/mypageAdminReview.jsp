...
<body>
                ...
                <form action="/adminDeleteReview">
                    ...
                    <div class="page-nav">
                        <c:if test="${pageStart!=1}">
                            <a href="mypageAdminReview?searchType=&keyword=${param.searchType}&keyword=${param.keyword}&reqPage=${pageStart-1}&list_num=${param.list_num}}">이전</a>
                        </c:if>
                        <c:forEach var="i" begin="${pageStart}" end="${pageEnd}">
                            <a href="mypageAdminReview?searchType=${param.searchType}&keyword=${param.keyword}&reqPage=${i}&list_num=${param.list_num}"
                                style="${i==(not empty param.reqPage ? param.reqPage : 1) ? " color: black;" : ""
                                }">${i}</a>
                        </c:forEach>
                        <c:if test="${pageEnd<pageSize}">
                            <a href="mypageAdminReview?searchType=${param.searchType}&keyword=${param.keyword}&reqPage=${pageEnd+1}&list_num=${param.list_num}">다음</a>
                        </c:if>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        $(function () {
            $(".admin-nav a:eq(2)").css("background-color", "#F2C335");
            setClickToTr();
            setClickToChk()
