...
public class MypageAdminShopServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(type == 1 && !keyword.equals("")) { //id 검색일 경우
			...
			totalCount = new HairshopService().totalCount();
			pageSize = (totalCount/5) + (totalCount%5 != 0 ? 1 : 0);
			list = new HairshopService().hairshopMore((reqPage*5)-4);
		}
		int maxSize = 5;
